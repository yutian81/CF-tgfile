...
export async function getFileByUrl(config, url) {
    return await config.database.prepare(
    `SELECT fileId, message_id, file_name, mime_type, url
    FROM files WHERE url = ?`
    ).bind(url).first();
}

// ✅ 新增：通过文件名获取记录（支持覆盖逻辑）
export async function getFileByName(config, name) {
    return await config.database.prepare(
    `SELECT url, fileId, message_id, file_name, mime_type
    FROM files WHERE file_name = ? COLLATE NOCASE
    LIMIT 1`
    ).bind(name).first();
}
