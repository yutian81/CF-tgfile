// src/handlers/file-service.js

import { insertFile, getAllFiles, searchFiles, getFileByUrl, deleteFile as dbDeleteFile, getFileByName } from './database.js';
...
export async function uploadFileToTelegram(file, config) {
  ...
  // --- 检查同名文件 ---
  const existingFile = await getFileByName(config, file.name);
  if (existingFile) {
    try {
      // 删除 Telegram 上旧消息
      await fetch(
        `https://api.telegram.org/bot${config.tgBotToken}/deleteMessage?chat_id=${config.tgChatId}&message_id=${existingFile.message_id}`,
        { method: 'POST' }
      );
    } catch (err) {
      console.warn('[Telegram Delete Warning]', err.message);
    }
    // 删除数据库记录
    await dbDeleteFile(config, existingFile.url);
  }
  ...
  if (!fileId) throw new Error('无法从Telegram获取文件ID');
  if (!messageId) throw new Error('无法从Telegram获取消息ID');
...
export async function deleteFileService(config, fileUrl) {
  ...
  try {
    ...
    if (!deleteResponse.ok) {
      ...
      throw new Error(`TG消息删除失败: ${errorData.description}`);
    }
  } catch (error) { 
  ...
  return {
    success: true,
    message: deleteError ?
      `文件已从数据库中删除，但TG消息删除失败: ${deleteError}` :
      '文件删除成功'
  };
}

// 共享的单文件获取逻辑
export async function getFileByUrlService(config, fileUrl) {
  return await getFileByUrl(config, fileUrl);
}